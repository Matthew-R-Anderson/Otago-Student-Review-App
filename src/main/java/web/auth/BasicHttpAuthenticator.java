	...
	public void configure(Env env, Config conf, Binder binder) throws Throwable {
		...
		router.use("*", "*", (req, rsp, chain) -> {
			...
			try {
				...
				Matcher matcher
					= Pattern.compile("(?<Student_ID>.+?):(?<Password>.*)")
						.matcher(authDetails);
				...
				String Student_ID = matcher.group("Student_ID");
				String Password = matcher.group("Password");

				if (validator.validateCredentials(Student_ID, Password)) {
					// add username to request
					req.set("Student_ID", Student_ID);
					chain.next(req, rsp);
				} else {
					// bad credentials
					rsp.send(new Result().header("WWW-Authenticate", "None").status(Status.UNAUTHORIZED));
